    ...
    <Sheet open={open} onClose={() => setOpen(false)} heading="Settings">
      ...
      <div className={f.group}>
        <input
          ...
          onChange={(e) => updateConfig({ names: [e.target.value, config.names[1]] })}
          placeholder="You"
        />
        <input
          className={f.input}
          value={config.names[1]}
          onChange={(e) => updateConfig({ names: [config.names[0], e.target.value] })}
          placeholder="Them"
        />
      </div>
      ...
      {space && !space.partner2Id && (
        <>
          <span className={f.label}>Your space</span>
          <div className={f.group}>
            <button
              type="button"
              className={f.listRow}
            ...
            >
              <span className={f.rowLabel}>Invite someone</span>
      ...
      <div className={f.row}>
        ...
        >
          {authPhase === 'signedIn' ? 'Reconnect' : 'Connect'}
        </button>
      </div>
    </Sheet>
  );
